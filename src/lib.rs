...
//! - Return types are obviously API, and that includes **errors**.
//! - What _traits_ the API types implement is API. For example, dropping `Clone` from a type would
...
//! support the case where internal selection improvements are made without requiring a new major.
