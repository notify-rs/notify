...
fn test_poll_watcher_distinguish_modify_kind() {
  ...
  harness.expect_recv(&testfile, EventKind::Modify(ModifyKind::Metadata(MetadataKind::WriteTime)));
  harness.advance_clock();

  harness.write_file_keep_time(&testfile, "data2");
  harness.expect_recv(&testfile, EventKind::Modify(ModifyKind::Data(DataChange::Any)));
  harness.advance_clock();
...
impl TestHarness {
  pub fn setup() -> Self {
    ...
    let watcher = PollWatcher::with_config(move |event: notify::Result<Event>| {
      println!("event={event:?}");
  ...
  pub fn watch_tempdir(&mut self) {
    self.watcher.watch(self.testdir.path(), RecursiveMode::Recursive).unwrap();
  }
  ...
  fn expect_recv<P: AsRef<Path>>(
    &self,
    expected_path: P,
    expected_kind: EventKind) {
    let actual = self.rx.recv_timeout(Duration::from_secs(15)).unwrap()
